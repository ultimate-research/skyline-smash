...
Ideally, the framework should allow users to specify these four items in order to replace a specific ACMD script with one the user provides. An example of defining what ACMD script a function would replace might look something like the following:
```lua
#[skyline::acmd(*BATTLE_OBJECT_CATEGORY_FIGHTER, *FIGHTER_KIND_MARIO, "attack_lw4", "game_attacklw4")] 
//This refers to Mario's downsmash "game" ACMD script
```

ACMD Terminology/Glossary:
- `ACMD Script`  a function that contains the code relevant to a certain "motion". This could be related to hitboxes and general game mechanics (game), sound (sound), effects (effect), and expressions (expression).
- `Article`  an "item" in-game that isn't necessarily a true item. Things like Ganondorf's sword, snake's C4, and Mario's fireball are all articles. These too have ACMD scripts attacked to them that run based on the current motion. For example, Snake's C4 has an ACMD script called "Explosion" that handles (among a few other things) the hitbox for the detonation of his C4.
 
- `is_excute`  a function that returns (bool) whether or not the character is on the frame that has been declared previously or not.
- `frame`  a function that takes in a frame value, and using that, determines when the next is_excute should return true.
- `wait`  a function that takes in a number of frames, and using that, determines when the next is_excute should return true.

Example of the 3 above concepts/functions in the context of a theoretical framework:
```lua
frame(5); //any is_excute below this statement will happen on frame 5
if(is_excute(module_accessor)) { //checks the current frame of the current animation to see if it complies with the previous frame declaration
  ATTACK(param_1, param_2, param_3, ...);
  More_Functions_Here();
}
wait(2); //this waits 2 frames past the previous frame declaration. Since the previous declaration was 5, the next is_excute(s) will happen on frame 7
if(is_excute(module_accessor)) { //frame 7
  More_Functions_Here();
{
```
...
- Some scripts don't have frame declarations before an is_excute. What happens then?
    - In that case the is_excute would default to checking for frame 1.
...
- Porting from old ACMD framework -
  - Many people have ACMD written in the old framework and will likely want to bring it over as painlessly as possible. This could likely be acomplished with a Python script that would convert the ACMD object headers to Skyline-RS ACMD headers and plop the contained code into its own function likely with an auto-generated name based on the motion_kind and acmd_type. Any broad changes made to the normal flow of function calls should also be taken into account: I.E. any occurance of "acmd->module_accessor" could be converted into the Skyline-RS equivalent. This would not be a perfect script, and would simply get the general stuff ported, leaving the specifics of fixing special-case situations up to the user.
